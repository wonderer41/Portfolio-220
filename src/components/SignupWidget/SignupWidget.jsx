...
const SignupWidget = ({ title, content, simulateNetworkRequestTime }) => {
  ...
  return (
    <form
      className={
        "flex flex-col p-6 border border-zinc-100 dark:border-zinc-700/40 rounded-2xl w-full lg:w-[398px] h-[196px] gap-x-3"
      }
    ...
    >
      <h2 className="flex flex-row items-center font-bold text-zinc-900 dark:text-zinc-100 gap-3 m-0 leading-7">
        <svg
          className="stroke-zinc-400 fill-zinc-100 dark:fill-zinc-100/5"
          width="22"
          height="18"
          viewBox="0 0 22 18"
      ...
      {isSubscribed ? (
        <p className="w-[350px] h-48 pt-2 text-zinc-600 dark:text-zinc-400 text-sm leading-6">{message}</p>
      ) : (
        <>
          ...
          <div className="flex flex-row w-full lg:w-[350px] h-[64px] pt-6 gap-4">
            {
              <input
                className="p-2.5 flex lg:w-[282px] h-10 dark:bg-zinc-700/20 border border-zinc-900/10 dark:border-zinc-700 rounded-md"
                type="email"
            ...
            {
              <button
                className="px-2 py-3 w-[53px] h-[40px] bg-zinc-800 dark:bg-zinc-700 flex justify-center items-center rounded-md text-zinc-100 leading-4 text-sm gap-1"
                type="submit"
                disabled={busy}
              >
                {busy ? "Sending..." : "Send"}
